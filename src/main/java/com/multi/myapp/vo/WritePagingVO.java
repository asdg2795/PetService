...
// to string은 가능
@ToString
public class WritePagingVO {
//	pvo의 역할 - 전체글을 가지고 오는 것이 아니라 
//	하나의 단위로 묶어서 가져온다
	
	private int nowPage = 1;	//현재페이지
	private int onePageRecord = 5;//한번에 표시할 레코드 수
	
	private int writetotalRecord;	//총레코드 수
	...
	public int getTotalRecord() {
		return writetotalRecord;
	}

	public void setTotalRecord(int writetotalRecord) {
		this.writetotalRecord = writetotalRecord;
		// 총페이지수
		totalPage = (int) Math.ceil( writetotalRecord / (double)onePageRecord);
		/*
